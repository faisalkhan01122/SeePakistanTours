...
const Section = ({ title, content, image, reverse }) => (
  <div className={`flex flex-col md:flex-row items-center gap-8 py-12 ${reverse ? "md:flex-row-reverse" : ""}`}>
    ...
    <motion.div
      ...
      transition={{ duration: 0.6 }}
    >
      <h2 className="text-2xl md:text-3xl font-bold text-primary-600 mb-4">{title}</h2>
      {content.map((p, idx) => (
        <p key={idx} className="text-gray-600 text-base md:text-lg mb-3 leading-relaxed">{p}</p>
      ))}
    </motion.div>
  </div>
);
  ...
  return (
    ...
    {/* ------------------------SubHero----------------------------- */}
      ...
      <Section
        ...
        content={[
          "Hunza Valley tours are the best demanded tours by travelers to explore in Hunza tour packages 2025 by See Pakistan Tours. Under the shadows of mighty mountains, Hunza Valley lies in Gilgit-Baltistan.",
...
<HunzaPlacesSection/>
      ...
      >
        ...
        <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
          Choose from our curated Hunza Valley Tour Packages based on your interests and budget. Plan now and avail early bird discounts!
        </p>
